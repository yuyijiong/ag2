...
# ---------------------------------------------------------------------------
# Storage protocol + MemoryStorage (unchanged)
# ---------------------------------------------------------------------------
...
class LongShortTermMemoryStorage:
    ...
    **Custom consolidation logic**
    ...
    def __init__(
        ...
        *,
        ...
        config:
            ...
            built-in summariser and consolidator.  Required unless both
            *summarizer* and *consolidator* are supplied explicitly.
        summarizer:
        ...
        l2_store_path:
            ...
            Both L2 blocks **and** the L1 blocks they reference are stored so
            that ``memory_lookup`` can retrieve full source text in any future
            session.  L1 blocks that are no longer referenced by any L2 block
            are pruned on each save to prevent unbounded growth.
    ...
    async def consolidate(self) -> None:
        """Consolidate all pending L1 blocks into L2 and persist to disk.
        ...
        Before running L2 consolidation, any raw events that have not yet been
        compressed (because the token threshold was never exceeded) are
        force-flushed into L1 blocks so the end of the conversation is never
        silently discarded.

        The consolidator callable returns a list of ``L2Operation`` objects
        describing only the changes.  ``_apply_l2_ops`` then applies them to
        the current L2 state.

        If there are no pending L1 blocks the method is a no-op.
        """
        # Ensure the tail of the conversation (below the token threshold) is
        # captured in L1 before we attempt L2 consolidation.
    ...
    def _build_memory_header(self, stream_id: "StreamId") -> ModelRequest | None:
        ...
        lines: list[str] = ["[Memory Context — use memory_lookup(block_id) for full details]"]

        if l2:
            lines.append("\n### Long-Term Memory (L2 Blocks)")
            for block in l2:
                lines.append(f"  [{block.id}] (created {block.created_at}) {block.abstract}")

        if l1:
            lines.append("\n### Short-Term Memory (L1 Blocks)")
            for block in l1:
                lines.append(f"  [{block.id}] (created {block.created_at}) {block.abstract}")
    ...
    def _load_store(self) -> None:
        """Load L1 and L2 blocks from the JSON store file, if it exists.
        ...
        L2-only (no persisted L1 blocks) for backward compatibility.
        """
